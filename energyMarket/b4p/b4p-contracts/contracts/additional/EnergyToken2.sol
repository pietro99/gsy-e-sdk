...
*/
    function addEnergyBatch(uint _energy, uint _amount) external onlyOwner{
 ...
 */
    function setForSale(uint256 _id, uint _amount, uint _price) hasToken(msg.sender, _id, _amount) public {
        tokensPrice[msg.sender][_id] = tokenForSale(_price,_amount); 
    }

    /**
    * @dev See {IERC1155-safeTransferFrom}.
    */
    function safeBuy(uint256 _id,uint256 _amount, address _from) hasToken(_from, _id, _amount)public payable {
        require(tokensPrice[_from][_id].amount >= _amount, "not so many tokens are for sale");
        require( msg.value == tokensPrice[_from][_id].price*_amount, "wrong value sent");
        address _to = msg.sender;
        bytes memory _data = "";
        tokensPrice[_from][_id].amount = tokensPrice[_from][_id].amount - _amount;
        balances[_from].ethBalance += msg.value;
...
*/
   ...
   function getLatestPrice(AggregatorV3Interface priceFeed) public view returns (int) {
    ...
    modifier hasToken(address _from, uint _id, uint _amount) {
        
        require(balanceOf(_from, _id) >= _amount, "sender does not own correct amount of tokens");
        _;
   }
    /**
