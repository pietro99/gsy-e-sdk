...
*/
    ...
    function produce(address producer, uint _amount) external {
        if(addressToId[producer] == 0){
            counter.increment();
            addressToId[producer] = counter.current();
        }
        _mint(producer, addressToId[producer], _amount, "");
    }

    function transferFrom(address _from, address _to, uint _amount) external {
        bytes memory _data = '';
        uint _id = addressToId[_from];
